...
.PHONY: help legacy-scalping test-legacy test-config test \
	...
	hermes-ops-setup ops-monitor ops-monitor-once
...
ops-monitor: ## 운영 머신 상태 서브에이전트 루프 (기존 TELEGRAM_* 재사용)
	@$(PYTHON) scripts/ops_subagent_monitor.py $(ARGS)

ops-monitor-once: ## 운영 머신 상태 1회 보고 (기존 TELEGRAM_* 재사용)
	@$(PYTHON) scripts/ops_subagent_monitor.py --once $(ARGS)
