...
_CLOSE_RUSH_END = os.environ.get("BARRO_CLOSE_RUSH_END", "1520").strip()


def _close_rush_active() -> bool:
    """종가러시 윈도우(평일, default 15:00~15:20) + 토글 ON 이면 True → 데몬 신규진입 양보."""
    if not _CLOSE_RUSH_THROTTLE:
        return False
    n = _now_kst()
    if n.weekday() >= 5:
        return False
    try:
        st = time(int(_CLOSE_RUSH_START[:2]), int(_CLOSE_RUSH_START[2:]))
        en = time(int(_CLOSE_RUSH_END[:2]), int(_CLOSE_RUSH_END[2:]))
...
def _manual_hold_held() -> set[str]:
    """수동관리 보유 종목 — 자동 손절/청산 제외(사용자 직접 매도). data/manual_hold_symbols.json.
    [사용자 요청 2026-06-22] 예스티(122640) 등 수동 손절 종목을 자동매도(장중평가·carry-limit·EOD)에서 제외."""
    try:
        d = json.loads((_DATA_DIR / "manual_hold_symbols.json").read_text(encoding="utf-8"))
...
async def _scan_and_buy(
    ...
    if _now_kst().time() < BUY_START:
        return 0
    # [6/24] 종가러시 throttle — 종베 API 우선권 위해 데몬 신규진입 스캔 양보(429 완화).
...
async def _eod_carry_limit(args, oauth, notifier) -> int:
    ...
    _cb_skip = _closing_bet_held() | _manual_hold_held()  # 종베 + 수동관리 보유분 EOD 강제청산 제외
    sold = 0
