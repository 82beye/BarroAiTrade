#!/usr/bin/env python3
"""f_zone 전략 정밀 분석 — 8 운영 종목 × 600봉 (일봉).

목적:
  REPORT_600BARS 에서 f_zone 만 total_pnl 음수(-99,409). 어느 종목에서 어떻게
  손실이 나는지, winning vs losing 패턴 차이가 있는지 정밀 분석.

산출:
  - stdout: 종목별 ranking, exit reason 분포, win/loss 분포
  - FZONE_ANALYSIS.md: 마크다운 보고서
  - fzone_all_trades.csv: 모든 trade entry-exit pair
"""
...
REPORT_MD = OUT_DIR / "FZONE_ANALYSIS.md"
TRADES_CSV = OUT_DIR / "fzone_all_trades.csv"
...
def trade_metrics(p: dict) -> dict:
    ...
    sell_value = sum(e["price"] * e["qty"] for e in p["exits"])
...
def analyze_symbol(symbol: str, name: str) -> dict:
    ...
    total_pnl = float(result.pnl_by_strategy.get("f_zone", 0))
...
def build_report(all_results: list[dict]) -> str:
    out = StringIO()
    out.write("# f_zone 정밀 분석 — 8 운영 종목 × 600봉\n\n")
    out.write(f"_시뮬: IntradaySimulator (TP +3/+5/+7%, SL -1.5%, commission 0.015%/leg, tax 0.18%)_\n\n")
...
def main() -> int:
    ...
    for sym, name in SYMBOLS.items():
        try:
            ...
            print(f"[OK] {sym} {name:<10} trades={r['n_trades']:>2} "
                  f"win={r['wins']:>2} pnl={fmt_money(r['total_pnl'])}")
