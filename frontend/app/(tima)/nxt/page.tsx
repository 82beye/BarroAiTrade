...
// ── NXT 애프터마켓 임시 비활성 ──
// nav 에서는 이미 제거됨. 직접 URL 접근 시에도 안내 카드만 노출한다.
// 게이트웨이(넥스트레이드) 연동 복원 시 하단 보존 블록의 기존 구현으로 되돌린다.
export default function NxtPage() {
  return (
    <div className="p-3">
      <div className="rounded-lg border border-tima-line bg-white px-4 py-12 text-center">
        <p className="text-base font-bold text-tima-text">NXT 애프터마켓은 임시 비활성입니다</p>
        <p className="mt-2 text-sm leading-relaxed text-tima-sub">
          대체거래소(넥스트레이드) 게이트웨이 연동 후 복원 예정입니다.
        </p>
      </div>
      <Disclaimer />
    </div>
...
/* ──────────────────────────────────────────────────────────────────────────
 * [보존] 기존 NXT 실구현 — 게이트웨이 복원 시 아래 코드로 되돌린다. (TODO: restore)
 * 키움 랭킹 TR stex_tp=2(NXT) 기반. api.getNxt(filter, limit) 사용.
 *
 * 'use client';
 *
