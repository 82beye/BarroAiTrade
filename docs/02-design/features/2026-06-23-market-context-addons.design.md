...
- **LLM 오버레이(opt-in)**: `agent_advisory_writer.market_context_llm_overlay` — 결정적 regime base 위에 `claude -p`로 risk_on/confidence/strategy_gates/reason 판단을 덮어쓴다. writer `--market-llm`(또는 env `BARRO_MARKET_LLM`)일 때만, 실패/응답불가 → 결정적 base 로 **fail-open**. regime·ts·테마 집계는 보존(LLM은 soft 신호만).
...
| sector_themes | hot_themes(거래대금) | 거래대금 집중 핫테마 중 **under-exposed 신호 우선순위 가점**(soft 재정렬, `apply_sector_priority`) → 슬롯 경합 시 우선 매수 | `sector_themes_enabled`·`_mode`·`sector_underexposed_max_pct`(0.30)·`sector_min_turnover_pct`(0.0) |
...
## 8. 한계 / 후속
- `theme_map.json` 커버리지 = 가드 정확도(미매핑 fail-open).
- **LLM 오버레이 = 구현됨**(market_context, `--market-llm` opt-in). 단 현재는 *스냅샷(regime+거래대금+테마노출)* 종합 판단 — 뉴스-인지 거시(WSJ/공시 파이프라인)는 후속.
- **sector priority 배선 = 구현됨**(`apply_sector_priority`, under-exposed 핫테마 우선 재정렬).
- **Telegram 표시 = 구현됨**(`format_macro/sector/portfolio_alert`, writer `--telegram` 시 시장국면·핫테마·포트폴리오 요약, 변경 시만 전송). leverage 판정(balance), shadow add-on 확장은 후속 증분.
