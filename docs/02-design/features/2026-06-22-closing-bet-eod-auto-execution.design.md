...
### 6.1 Addendum (2026-06-22) — 1차 dry-run(페이퍼/알림) 통과 기준
구현 착수(§5) 전제. **4개 모두 충족 시 "통과"** → 구현 착수. 측정원: `data/closing_bet_paper.csv` 익일가 집계 + `_daily_strategy_audit`(브로커 실측 시). 평가는 이격도 게이트 ON(`BARRO_CB_DISPARITY_YELLOW=1`) 기준.

| # | 기준 | 합격선 |
|---|------|--------|
| 1 | **표본** | 관찰 1~2주에 종베 매수신호 **≥ 10건**(통계 의미) |
| 2 | **net 수익** | 익일가 기준 평균 **net > 0**(왕복 0.90% 차감). 이상적 **≥ +0.4%**(OOS 이격도 ON 수준 +0.405%) |
| 3 | **적중·whipsaw** | 익일 슈팅(09~10시) 도달이 손실신호 대비 우위, 명백한 오신호 없음 |
| 4 | **운영 무사고** | 알림·포지션 등록 정상(중복/누락 없음) |

- 미달(특히 #2 net≤0 — '종가진입 브레이크이븐' OOS caveat 현실화) → **구현 보류**, 파라미터(이격도 임계·진입창·money_flow) 재검토 또는 종베 자동매매 폐기 판단.
- 통과 후에도 §6 순서대로 ②`AUTOEXEC=1 --dry-run`(주문 로직) → ③ sim정합 → ④ 라이브 ★HITL 소액 1종목.
