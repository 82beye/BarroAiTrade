...
전제: 아래 내용은 main 워킹트리의 **미커밋 상태**를 기준으로 함(git status 참조). 이 세션과는 별도로 동시에 진행 중이던 작업으로 추정됨.
...
```text
theme_board_cache_jobs        20s   ka10001 (종목별, enrich=True 일 때)
...
3. `theme_market_rows_capture`(60s)와 `theme_board_cache_jobs`(20s)는 **같은 랭킹 데이터(top-N value/gainers)를 사실상 중복 호출**한다. `theme_board_cache_jobs`가 개별 `ka10001`(종목당 1회)을 쓰는 반면 `market_row_store`는 `ka10032`/`ka10027`(전체 한 번에), TR이 달라 완전한 중복은 아니지만, §2-D의 통합안으로 호출 총량을 줄일 수 있다.
...
현재 §2에서 다룬 모든 변경(신규 파일 3개 + 수정 파일 다수, 프론트 포함)은 **main 워킹트리에 미커밋 상태**다. 이 세션은 이 변경들을 만든 당사자가 아니며(동시 진행 중이던 별도 작업으로 추정), 임의로 커밋·수정하지 않았다. 이 파일들은 현재 실행 중인 백엔드 서비스가 실제로 참조하고 있으므로:

- 이 상태에서 `git checkout .`/`git reset --hard` 등을 실행하면 **동작 중인 코드가 사라진다** — 절대 금지.
- 위 §3 수정안을 실제로 적용하려면, 이 미커밋 변경을 누가 커밋할지(원 작성자 확인 필요) 먼저 정리한 뒤 그 위에 패치를 얹어야 한다.
- 데이터 손실 우려: `data/barro_trade.db`도 modified 상태로 잡혀 있다 — DB 파일 자체를 git으로 관리하는 것은 별개 위생 문제이나(이전 세션에서도 언급됨), 지금 시점에 그 파일을 git 작업으로 되돌리면 라이브 SQLite 상태가 깨진다.
