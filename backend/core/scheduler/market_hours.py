...
def _as_kst(now: Optional[datetime]) -> datetime:
    if now is None:
...
def is_regular_market(now: Optional[datetime] = None) -> bool:
    """평일 KRX 정규장/종가 단일가 구간(09:00~15:30 KST) 여부."""
    current = _as_kst(now)
...
__all__ = ["is_open_rush", "is_regular_market"]
