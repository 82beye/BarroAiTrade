...
# 테마로만 한정한다. themes.name 은 UNIQUE 라 이름이 우연히 겹치면(예: 큐레이션의
# "방산") 항상 upsert 경로를 타야 하므로 replace 여부와 무관하게 upsert 로 통일한다.
    ...
    *,
    ...
    async with get_db() as db:
        ...
        for item in snapshot.get("themes") or []:
            ...
            # upsert-by-name — replace 와 무관하게 항상 이 경로. 다른 출처와 이름이
            # 겹치면(예: 큐레이션의 "방산") 그 테마를 이어받아 description 만 갱신한다
            # (해당 theme_id 는 Finup 마커가 없으므로 다음 재수입 때도 보호된다).
            if is_sqlite:
                await db.execute(
                    text(
