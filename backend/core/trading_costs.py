...
근거: reports/2026-06-11/2026-06-11_매매복기.md 핵심분석 1.
6/11 브로커 체결 실측(fill_audit.csv, ka10073)에서 수수료가 **왕복 거래대금의
0.349%(편도 ~0.175%)**, 거래세가 매도대금의 0.200%로 확인됐다 — 기존 코드 전반의
가정(편도 0.015% + 세 0.18%)의 11.6배. 이 격차가 ①6/11 gross +67K 흑자를 net
-120K 적자로 전환시켰고 ②추정 손익의 누적 낙관 편향 -1.79M 중 85%를 만들었다.

단위 규약 (일괄 치환 금지 — 100배 오차 함정):
- *_RATE  = 소수 단위 (0.00175 = 0.175%). 일일감사·이브닝 파이프라인·ob_scalp 등.
- *_PCT   = 퍼센트 단위 (0.175 = 0.175%). backend/core/backtester/* 생성자 인자 전용
...
# 편도 수수료율 (소수). 실측: 6/11 fill_audit 51행 — 145,520 / (매수 20.79M + 매도 20.86M)
#   = 왕복 0.3494% → 편도 0.00175. 협의/우대 요율 적용 시 env 로 내릴 것.
COMMISSION_RATE = Decimal(os.environ.get("BARRO_COMMISSION_RATE", "0.00175"))
...
COMMISSION_PCT = float(COMMISSION_RATE * 100)   # 0.175
