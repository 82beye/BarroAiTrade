    ...
    *,
    ...
    for sid in strategy_ids:
        if sid == "f_zone":
            ...
            # BAR-OPS-09 Phase 8: 진입 점수 임계 강화 (min_score=5.0, 0-10 스케일) — 약한 시그널 차단.
            # default 는 회귀 보존 (min_atr_pct=0.0, min_score=3.0 = 기존 임계 × 10).
            # 5/22 LG전자 -148k / 삼성전기 -124k 같은 약한 swing 진입 시뮬 차단 기대.
            out.append(Swing38Strategy(Swing38Params(min_atr_pct=0.035, min_score=5.0)))
