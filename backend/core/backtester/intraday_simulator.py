        ...
        for strategy, sid in zip(strategies_obj, strategy_ids):
            ...
            for i in range(self._warmup, len(candles)):
                ...
                if pending_entry and position is None:
                    ...
                    current_plan = _exit_plan_for_strategy(sid, entry_price, window, early_tp=self._early_exit_tp)
...
def _scaled_exit_plan(
    ...
    default (early_tp=False): TP +3/+5/+7%, qty 0.33/0.33/0.34 — baseline 보존.
    early_tp=True (BAR-OPS-09 Phase B, 2026-05-23): TP +1.5/+3/+5%, qty 0.3/0.35/0.35.
    작은 익절 잠금으로 승률 향상 (5/22 LG전자 +4.02% 같은 케이스 잡힘).
    """
    if early_tp:
        return ExitPlan(
            take_profits=[
...
def _exit_plan_for_strategy(
    ...
    early_tp=True (BAR-OPS-09 Phase B): TP 첫 tier 임계 절반 + qty 0.30 → 작은
    익절 잠금으로 승률 향상. baseline 보존을 위해 default False.
    """
    if strategy_id == "sf_zone":
        return _sfzone_atr_exit_plan(entry_price, candles_window, early_tp=early_tp)
    return _scaled_exit_plan(entry_price, early_tp=early_tp)
