...
async def force_close_all(reason: str = Query("manual", description="강제청산 사유")) -> dict:
    ...
    # BAR-171: orchestrator.force_close_all() 직접 호출 — 실제 시장가 매도 실행.
    # 기존 코드는 WS broadcast만 하고 실행 주체가 없어 청산이 되지 않는 버그였음.
    try:
        from backend.core.orchestrator import orchestrator
        await orchestrator.force_close_all(reason=f"수동 강제청산: {reason}")
    except Exception as e:
        logger.error("강제청산 실행 실패: %s", e)
        raise HTTPException(status_code=500, detail=f"강제청산 실패: {e}")

    logger.warning("수동 강제청산 실행: %s개 종목 (%s)", len(symbols), reason)
    return {
        "status": "ok",
        "message": f"{len(symbols)}개 포지션 청산 완료",
        "symbols": symbols,
        "reason": reason,
    }
