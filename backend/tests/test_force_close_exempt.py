...
청산될 수 있었다. _FORCE_CLOSE_EXEMPT_STRATEGIES + _force_close_skip() 로 명시 보존한다.
...
def test_none_strategy_not_skipped_when_not_closing_bet():
    """strategy 미상 + 종베 아님 → 제외 안 함(강제청산 대상)."""
    assert _force_close_skip("005930", strategy=None, cb_skip=set()) is False
