...
class TestFinupImporterSourceScoping:
    ...
    async def test_name_collision_with_non_finup_theme_upserts_not_crashes(
        ...
        assert rows[0]["description"] == "큐레이션 시드"
        assert symbols == {"079550", "012450"}  # 기존+신규 종목 공존

    @pytest.mark.asyncio
    async def test_later_replace_never_deletes_name_collision_theme(self, theme_db, tmp_path):
        repo = ThemeRepository()
        existing_id = await repo.upsert_theme("방산", description="큐레이션 시드")
        await repo.link_stock(existing_id, "079550", 5.0)

        collision = _write_snapshot(tmp_path, [_theme_item("방산", [("012450", 9.0)])])
        await import_finup_theme_snapshot(collision, update_stock_names=False)

        next_snapshot = _write_snapshot(tmp_path, [_theme_item("로봇", [("005930", 1.0)])])
        await import_finup_theme_snapshot(next_snapshot, update_stock_names=False)

        async with get_db() as db:
            theme_res = await db.execute(
                text("SELECT id, description FROM themes WHERE name = '방산'")
            )
            theme = theme_res.mappings().first()
            marker_res = await db.execute(
                text(
                    "SELECT COUNT(*) AS c FROM theme_keywords "
                    "WHERE theme_id = :tid AND keyword = '__source:finup__'"
                ),
                {"tid": existing_id},
            )
            symbols_res = await db.execute(
                text("SELECT symbol FROM theme_stocks WHERE theme_id = :tid"),
                {"tid": existing_id},
            )

        assert theme is not None
        assert theme["id"] == existing_id
        assert theme["description"] == "큐레이션 시드"
        assert marker_res.mappings().first()["c"] == 0
        assert {r["symbol"] for r in symbols_res.mappings().all()} == {"079550", "012450"}
